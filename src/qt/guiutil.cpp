...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("obsidian"))
        return false;
...
{
    // Convert obsidian:// to obsidian:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("obsidian://"))
    {
        uri.replace(0, 12, "obsidian:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Obsidian.lnk";
}
...
{
    return GetAutostartDir() / "obsidian.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Obsidian-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  obsidian-qt [" + tr("command-line options") + "]                     " + "\n";
