...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Obsidian is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Obsidian is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Obsidian is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Obsidian is downloading blocks...");
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    throw runtime_error(
            ...
            "Mining ODN is not supported > block 12500.");
